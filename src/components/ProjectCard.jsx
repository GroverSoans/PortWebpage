...
const ProjectCard = ({ title, image, description, githubLink, demoLink }) => {
  return (
    <div id="card" className="inline-block relative h-[475px] w-[400px] bg-[#2c3436] rounded-xl mx-4  " >
        <div className="h-1/2">
            <img className="w-full h-full p-2 object-cover rounded-t-2xl" src={image} alt="Project Image"/>
        </div>
        <div className="items-center ">
            <h2 className="text-white text-3xl font-bold my-1 text-center">{title}<span className='text-[#CFFF04]'>.</span></h2>
            <p className="text-white text-center px-2 text-wrap">{description}</p>
        </div>
